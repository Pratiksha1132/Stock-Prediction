...
company = input("Enter the tickername of the company: ")
...
prediction = prediction * 72.90
print(f"prediction: {prediction}")
# Predict for a month
predi = []
for x in range(predition_days, len(model_inputs)):
    predi.append(model_inputs[x - predition_days:x, 0])
predi = np.array(predi)
predi = np.reshape(predi, (predi.shape[0], predi.shape[1], 1))
pre = model.predict(predi)
